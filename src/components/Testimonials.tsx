...
import { Quote, Star, Plus, Send, Check } from 'lucide-react'
...
export default function Testimonials() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/xojrpbkq', {
        method: 'POST',
        headers: {
          ...
          >
            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <input
                    ...
                    className="px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-transparent outline-none text-gray-900 dark:text-white disabled:opacity-50"
                  />
                  <input
                    type="text"
                    name="role"
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-transparent outline-none text-gray-900 dark:text-white disabled:opacity-50"
                />
                ...
                <textarea
                  ...
                  disabled={isSubmitting}
                  rows={4}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-transparent outline-none text-gray-900 dark:text-white resize-none disabled:opacity-50"
                />
                ...
                <div className="flex space-x-4">
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="flex-1 flex items-center justify-center space-x-2 px-6 py-3 bg-gradient-to-r from-primary-600 to-accent-600 text-white rounded-lg font-semibold hover:opacity-90 transition-opacity disabled:opacity-50"
                  >
                    <Send size={18} />
                    <span>{isSubmitting ? 'Sending...' : 'Submit Testimonial'}</span>
                  </button>
                  <button
                    type="button"
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Your testimonial has been submitted successfully and will be reviewed soon.
                </p>
              </div>
            )}
          </motion.div>
